...
// 2. 모든 사용자 조회
const allUsers = db.prepare('SELECT * FROM users').all(); // all(), get() 등등...
console.log('모든사용자: ', allUsers);
...
const insert = db.prepare('INSERT INTO users (username, email) VALUES (?,?)').run(newUser.username, newUser.email);
console.log('추가된 사용자의 ID는: ', insert.lastInsertRowid);
